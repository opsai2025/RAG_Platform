...
def ask_rag(question, vectorstore):
    ...
    prompt = f""".  همچنین برای راهنمایی بدان اصولا در متونی قانونی و حقوقی ذیل هر ماده یکسری بند و تبصره می آید و اگر بند و تبصره شماره نداشت میتوانی بدین صورت ارجاع بدی که تبصره ذیل ماده فلان (آخرین ماده قبل از آن ماده یا بند). بر اساس متن زیر به سوال پاسخ بده. اگر پاسخ بصورت شفاف در متن نیست بگو نمی دانم ولی میتوانی برای راهنمایی کاربر به نزدیک ترین محتوا و پاراگرافی که در متن اشاره شده اشاره کنی ولی به کاربر بگی که این صرفا نزدیک ترین محتوا پیدا شده به سوال شما است
